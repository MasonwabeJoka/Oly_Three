...
const TextInputBar = forwardRef<HTMLTextAreaElement, Props>(
  (
    ...
    const validateInput = useCallback(
      (inputValue: string): { isValid: boolean; errors: string[] } => {
        const errors: string[] = [];
      ...
      <>
        ...
        <div className={`${styles.container} ${containerClassName}`} style={showShadow}>
          ...
          >
            ...
            <textarea
              ...
              aria-describedby={`${id}-description ${error || charCountError ? `${id}-error` : ""} ${characterLimit ? `${id}-char-count` : ""}`.trim()}
              aria-invalid={!!(error || charCountError || !isValid)}
        ...
        {characterLimit && (
          <div
            ...
            aria-live="polite"
          >
            {localValue.length}/{characterLimit} characters
          </div>
        )}
