import styles from "./AdSkeleton.module.scss";
...
const LandscapeAd: React.FC<SizeProps> = ({ cardSize, isDashboard }) => {
...
const ProfileAd: React.FC<SizeProps> = ({ cardSize, isDashboard }) => {
  const isSidebarOpen = useSidebarStore((state) => state.isSidebarOpen);
...
const AdSkeleton: React.FC<SkeletonProps> = ({ orientation, cardSize }) => {
  return (
    <div className={styles.container}>
      {orientation === "landscape" ? (
        <LandscapeAd cardSize={cardSize} />
      ) : (
        <ProfileAd cardSize={cardSize} />
      )}
    </div>
  );
};
