...
const VerificationSteps: React.FC<VerificationStepsProps> = ({ initialType, initialStep }) => {
  const router = useRouter();
  const [userType, setUserType] = useState<'individual' | 'business' | null>(initialType || null);
  ...
  const steps: Record<'individual' | 'business', StepType[]> = {
    individual: [
      { title: 'Verify Your Account', content: <IndividualAccountVerification />, path: 'account', fields: [] },
      { title: 'ID/Passport', content: <UploadId onNext={() => handleNext()} />, path: 'id-passport', fields: ['idFile'] },
      { title: 'Selfie Verification', content: <TakeSelfie onNext={() => handleNext()} />, path: 'selfie', fields: ['selfie'] },
      { title: 'Mobile Verification', content: <EnterMobileNumber onNext={() => handleNext()} />, path: 'mobile', fields: ['phoneNumber', 'verificationCode'] },
      { title: 'Finish', content: <Finish />, path: 'finish', fields: [] },
    ],
    business: [
      { title: 'Verify Business Account', content: <BusinessAccountVerification />, path: 'account', fields: [] },
      {
        title: 'Business Details Form',
        content: <EnterBusinessDetails onNext={() => handleNext()} />,
        path: 'details',
        fields: ['businessName', 'regNumber', 'taxNumber', 'street', 'suburb', 'city', 'province', 'postalCode'],
      },
      { title: 'Rep ID Verification', content: <UploadRepId onNext={() => handleNext()} />, path: 'rep-id', fields: ['repIdFile'] },
      { title: 'Rep Selfie Verification', content: <RepSelfie onNext={() => handleNext()} />, path: 'rep-selfie', fields: ['repSelfie'] },
      { title: 'Rep Mobile Verification', content: <EnterRepMobile onNext={() => handleNext()} />, path: 'rep-mobile', fields: ['repPhoneNumber', 'repVerificationCode'] },
      { title: 'Finish', content: <BusinessVerificationComplete />, path: 'finish', fields: [] },
    ],
  };
  ...
  useEffect(() => {
    if (initialType && initialStep) {
      ...
      const stepIndex = stepPaths[initialType as 'individual' | 'business'].indexOf(initialStep);
      if (stepIndex === -1) {
        router.push(`/dashboard/settings/account-verification/${initialType}/account`);
      } else {
        setUserType(initialType as 'individual' | 'business');
        setStep(stepIndex);
      }
    }
  }, [initialType, initialStep, router]);

  const handleUserType = (type: 'individual' | 'business') => {
    setUserType(type);
    setStep(0);
    methods.reset();
    router.push(`/dashboard/settings/account-verification/${type}/${steps[type][0].path}`);
  };
  ...
  const handleNext = async () => {
    ...
    if (isValid && step < steps[userType].length - 1) {
      ...
      setStep(nextStep);
      router.push(`/dashboard/settings/account-verification/${userType}/${steps[userType][nextStep].path}`);
    } else if (isValid && step === steps[userType].length - 1) {
      // Submit form data to backend
      try {
        const formData = methods.getValues();
        await axios.post('https://api.example.com/submit-verification', formData);
        router.push('/dashboard');
      } catch (error) {
        console.error('Submission failed:', error);
        methods.setError('root', { message: 'Submission failed. Please try again.' });
      }
    }
  };
  ...
  const handleBack = () => {
    if (step > 0 && userType) {
      ...
      setStep(prevStep);
      router.push(`/dashboard/settings/account-verification/${userType}/${steps[userType][prevStep].path}`);
    } else {
      setUserType(null);
      methods.reset();
      router.push('/dashboard/settings/account-verification');
