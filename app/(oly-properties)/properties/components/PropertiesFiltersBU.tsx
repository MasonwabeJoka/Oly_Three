...
const PropertiesFilter = ({ fields, selectedValues, onFilterChange }: PropertiesFilterProps) => {
  ...
  const hasSelection = fields.some((field) => (selectedValues[field.id] || []).length > 0);
    ...
    <div className={`${styles.container} ${hasSelection ? styles.hasSelection : ""}`}>
      <div className={styles.wrapper}>
        {fields.map((field) => {
          ...
          return (
            <div key={field.id}>
              {/* Field label so groups don't mix */}
              ...
              {field.options.map((option) => (
                <div
                  ...
                  role={field.mode === "single" ? "radio" : "checkbox"}
                  aria-checked={selected.includes(option.value)}
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      handleOptionClick(field, option.value);
                    }
                  }}
                >
                  {option.label}
